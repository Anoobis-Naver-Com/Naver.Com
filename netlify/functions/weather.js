...
// 기존 edge-functions/Weather.js 로직을 그대로 이식한 Netlify Function(Node)입니다.
// Edge Function 환경에서는 @netlify/blobs(Node 전용 패키지)가 정상 동작하지 않아
// 이 함수를 Node 런타임(Netlify Functions)으로 전환했습니다.
// 프론트(index.html)에서 호출하는 URL 경로(/api/weather)는 netlify.toml의
...
async function saveLatest(payload) {
    try {
        ...
        console.error('weather.js: Blobs 저장 실패', e && e.message);
    }
}
